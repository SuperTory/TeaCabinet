...
export default class ScrollImage extends Component{
  ...
  //渲染轮播图片
  renderBanner(){
    return this.props.bannerData.map((item,index)=>
  ...
  renderIndicate(){
    ...
    for (let i=0;i<this.props.bannerData.length;i++){
      ...
      if (i===index){
        jsx.push(<Text key={i} style={{fontSize:15,color:'#1fe0f3'}}>●</Text>)
      }else {
  ...
  setTimer(){
    this.timer=setInterval(()=>{
      this.setState((preState)=>{                     //更新pageIndex
        if(preState.pageIndex>=(this.props.bannerData.length-1)){      //如果页码越界则归零
          return {pageIndex:0}
        }else {
  ...
  bannerDetail(){
    let item=this.props.bannerData[this.state.pageIndex];
